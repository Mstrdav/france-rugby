...
ui <- page_navbar(
    ...
    layout_columns(
      ...
      card(
        ...
        card_body(
          ...
          plotOutput("plot_pca", height = "350px")
        )
      )
    ),
    card(
      ...
      card_body(plotlyOutput("plot_quotidien", height = "400px"))
    )
  ),
  ...
  nav_panel("Hypothèses",
    layout_columns(
      ...
      card(
        ...
        card_body(
          p("Les séances de type Match ou Rugby Collectif créent la fatigue musculo-squelettique la plus importante, se reflétant sur le score de Wellness global (échelle 1 à 5).", class = "text-muted", style="font-size:0.85em;"),
  ...
  nav_panel("Outils & Décision",
    navset_card_tab(
      ...
      nav_panel("📊 Rapport Post-Séance",
        p("Mesurer la performance lors de la dernière séance par rapport à la moyenne individuelle du joueur pour ce TYPE de séance exact."),
...
server <- function(input, output, session) {
  ...
  # ---- ONGLET 1 : EDA ----
  # Helper function to generate dynamic value_box
  ...
  # Reactive subset for 7 last days vs rest
  kpi_data <- reactive({
    df_f <- df_filtered()
  ...
  output$plot_quotidien <- renderPlotly({
    d <- df_filtered() %>% filter(!is.na(Date)) %>% arrange(Date)
  ...
  output$plot_h2 <- renderPlotly({
    df_plot2 <- df %>% drop_na(Wellness, Type)
    p <- ggplot(df_plot2, aes(x = Type, y = Wellness, fill = Type)) +
      geom_boxplot(alpha=0.8) +
      theme_minimal() + 
      theme(axis.text.x = element_blank()) +
      labs(x = "Type de Séance", y = "Score Wellness (1 à 5)")
    ggplotly(p)
  })
  ...
  output$table_readiness <- renderDT({
    ...
    df_dash <- df %>%
      ...
      left_join(
        df %>% group_by(Joueur) %>% summarise(Q1 = quantile(Wellness, 0.25, na.rm=TRUE),
                                              Med = median(Wellness, na.rm=TRUE)),
        by = "Joueur"
      ) %>%
      mutate(
        Statut = case_when(Wellness <= Q1 ~ "ROUGE", Wellness <= Med ~ "ORANGE", TRUE ~ "VERT"),
        Recommandation = case_when(
          Wellness <= Q1 ~ "Repos / Entraînement léger",
          Wellness <= Med ~ "Adapter la charge",
    ...
    datatable(df_dash, rownames = FALSE, options = list(pageLength = 15, dom = 't', scrollX = TRUE)) %>%
      formatStyle('Statut',
                  backgroundColor = styleEqual(c('ROUGE', 'ORANGE', 'VERT'), 
                                               c('#F2D7D5', '#FAE5D3', '#D5F5E3')),
                  fontWeight = 'bold') %>%
  ...
  output$table_alertes <- renderDT({
    alertes <- df %>% filter(!is.na(ACWR_dist)) %>%
      group_by(Joueur) %>% slice_tail(n=1) %>% ungroup() %>%
      mutate(
        Zone = case_when(ACWR_dist > 1.5 ~ "DANGER (> 1.5)", 
                         ACWR_dist > 1.3 ~ "ATTENTION (1.3 - 1.5)", 
                         ACWR_dist >= 0.8 ~ "OPTIMAL", 
                         TRUE ~ "SOUS-CHARGE (< 0.8)")
      ) %>%
      select(Joueur, `Poste groupe`, Date, ACWR_dist, Zone) %>% 
    ...
    datatable(alertes, rownames = FALSE, options = list(pageLength = 10, dom = 't', scrollX = TRUE)) %>%
      formatStyle('Zone', 
                  backgroundColor = styleEqual(c('DANGER (> 1.5)', 'ATTENTION (1.3 - 1.5)', 'OPTIMAL', 'SOUS-CHARGE (< 0.8)'),
                                               c('#F2D7D5', '#FAE5D3', '#D5F5E3', '#EAECEE')),
                  fontWeight = 'bold')
  })
  
  output$table_post_seance <- renderDT({
    # Moyennes individuelles historiques par type de séance
    moyennes <- df %>% group_by(Joueur, Type) %>%
      summarise(Dist_moy = mean(`Distance totale (m)`, na.rm=TRUE),
                Dist_HI_moy = mean(`Distance haute intensité (m)`, na.rm=TRUE),
                Contacts_moy = mean(`Nombre de contacts`, na.rm=TRUE),
                .groups = 'drop')
    
    # Prendre la DERNIÈRE séance existante pour chaque joueur
    derniere <- df %>% group_by(Joueur) %>% filter(Date == max(Date, na.rm=TRUE)) %>% slice_tail(n=1) %>% ungroup()
    
    rapport <- derniere %>%
      left_join(moyennes, by = c("Joueur", "Type")) %>%
      mutate(Ecart_Dist_Pct = round((`Distance totale (m)` - Dist_moy) / Dist_moy * 100, 1),
             Ecart_HI_Pct = round((`Distance haute intensité (m)` - Dist_HI_moy) / Dist_HI_moy * 100, 1),
             Synthèse = case_when(Ecart_Dist_Pct > 15 ~ "📈 Sur-performance relative", 
                                     Ecart_Dist_Pct < -15 ~ "📉 Sous-performance relative", 
                                     TRUE ~ "➡️ Dans la norme")) %>%
      select(Joueur, Type, `Distance totale (m)`, Ecart_Dist_Pct, `Distance haute intensité (m)`, Ecart_HI_Pct, Synthèse)
    
    datatable(rapport, rownames = FALSE, colnames = c("Joueur", "Type de séance", "Dist (m)", "Écart Dist (%)", "Dist HI (m)", "Écart HI (%)", "Synthèse Globale"),
              options = list(pageLength = 10, dom = 't', scrollX = TRUE)) %>%
      formatStyle('Ecart_Dist_Pct', color = styleInterval(c(-15, 15), c('red', 'black', 'darkgreen'))) %>%
      formatStyle('Ecart_HI_Pct', color = styleInterval(c(-15, 15), c('red', 'black', 'darkgreen')))
  })
